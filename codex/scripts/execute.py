...
class StepExecutor:
    ...
    def _load_guardrails(self) -> str:
        ...
        if agents_md.exists():
            sections.append(f"## 프로젝트 지침 (AGENTS.md)\n\n{agents_md.read_text(encoding='utf-8')}")
        docs_dir = ROOT / "docs"
        if docs_dir.is_dir():
    ...
    def _build_preamble(self, guardrails: str, step_context: str,
        ...
        return (
            f"당신은 Codex로 실행되는 {self._project} 프로젝트의 개발 에이전트입니다. 아래 step을 수행하세요.\n\n"
            f"{guardrails}\n\n---\n\n"
    ...
    def _build_codex_command(self) -> list[str]:
        cmd = [
            ...
            "--ask-for-approval", os.environ.get("CODEX_APPROVAL_POLICY", "never"),
            "--json",
            "--cd", self._root,
        ]
